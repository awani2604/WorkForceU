...
import { ShieldCheck, Menu, X, ArrowRight, User, Sparkles } from "lucide-react";
...
export const PublicNavbar = () => {
  ...
  return (
    <header className="sticky top-0 z-40 bg-[#141821] text-white border-b border-gray-800">
      {/* Role Quick-Switch Banner (Ideal for reviewers to easily jump between all 5 personas) */}
      <div className="bg-[#1e2330] border-b border-gray-800 px-4 py-1.5 text-xs text-gray-300 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-[#F2B705]">Demo Role Switcher:</span>
          <span>Jump to any module persona instantly:</span>
        </div>
        <div className="flex items-center gap-1.5">
          <button
            onClick={() => handleRoleQuickSwitch("customer")}
            className="px-2.5 py-0.5 rounded bg-[#2E6FB0] text-white hover:bg-[#24588D] font-medium transition cursor-pointer text-[11px]"
          >
            Customer
          </button>
          <button
            onClick={() => handleRoleQuickSwitch("professional")}
            className="px-2.5 py-0.5 rounded bg-[#C1502E] text-white hover:bg-[#A74123] font-medium transition cursor-pointer text-[11px]"
          >
            Professional
          </button>
          <button
            onClick={() => handleRoleQuickSwitch("trainee")}
            className="px-2.5 py-0.5 rounded bg-[#1D8C6C] text-white hover:bg-[#167056] font-medium transition cursor-pointer text-[11px]"
          >
            Trainee
          </button>
          <button
            onClick={() => handleRoleQuickSwitch("admin")}
            className="px-2.5 py-0.5 rounded bg-[#7C6BC4] text-white hover:bg-[#6755B1] font-medium transition cursor-pointer text-[11px]"
          >
            Admin
          </button>
        </div>
